...
<body>
	<div>
		...
		<nav>
			<!--logo 시작-->
			<div id="logo">
				<img src="<spring:url value='/resources/image/gclogo_.png'/>" alt="greencar_logo">
			</div>
			<!--logo 끝-->
	...
	<div id="wrap" class="wrap" name="wrap">
		...
		<div id="title_1" class="title_1" name="title_1">
			<a href="${contextPath}/news/newsList"> 뉴스 & 이슈</a>
		</div>
		<!-- 타이틀 (1) 끝-->
		...
		<div id ="news_sub" class="news_sub" name="news_sub">
			...
			<div id="news_1" class="news_box" name="news_1">
				<!--  사진 시작 -->
				<img src="<spring:url value='/resources/image/morning_img.jpg'/>"/>
				<!--  사진 끝 -->
							
				<!--  제목 시작 -->							
				<h3><c:out value="${news.news_title}"/></h3>
				<!--  제목 끝 --> 
				
				<!--  내용 시작 -->
				<p id="news_text_1" class="text" name="text_1">
				<c:out value="${news.news_text}"/></p>
				<!--  내용 끝 -->
			...
			<c:forEach begin="${newsPageMaker.startPage}" end="${newsPageMaker.endPage}" var="idx">
			<a class="pagging_button" href="newsList${newsPageMaker.makeQuery(idx)}">${idx}</a></c:forEach>
			</div>
